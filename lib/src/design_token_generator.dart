...
class DesignTokenGenerator {
  ...
  Future<void> generate() async {
    print('🎨 Starting design token generation...');

    // Load tokens
    print('📂 Loading design tokens from $assetsPath');
    ...
    print('✅ Design token generation completed successfully!');
  ...
  Future<void> _generateColorTokens(Map<String, dynamic> resolvedTokens,
    ...
    print('   ✓ Generated app_colors.dart');
  }
  ...
  Future<void> _generateSpacingTokens(Map<String, dynamic> resolvedTokens,
    ...
    await file.writeAsString(code);
    print('   ✓ Generated app_spacing.dart');
  }
  ...
  Future<void> _generateMainThemeFile(
    ...
    // Button theme'ini ekle eğer button renkleri varsa
    ...
    // Dark theme için de button theme'ini ekle
    if (buttonColors.isNotEmpty) {
      buffer.writeln('      elevatedButtonTheme: ElevatedButtonThemeData(');
      buffer.writeln('        style: ElevatedButton.styleFrom(');
      if (buttonColors['elevated'] != null) {
    ...
    print('   ✓ Generated app_theme.dart');
  }
  ...
  Map<String, String> _findBestSeedColors(Map<String, dynamic> resolvedTokens) {
    ...
    print('🔍 Found ${colorKeys.length} color tokens:');
    ...
    // Brand token'larını özellikle ara
    final brandTokens =
        colorKeys.where((key) => key.toLowerCase().contains('brand')).toList();
    print('🎯 Brand tokens found: ${brandTokens.length}');
    for (final token in brandTokens) {
      print('   Brand: $token -> ${resolvedTokens[token]}');
    }

    // Öncelik sırası: brand -> primary -> blue -> green -> purple -> diğerleri
    final priorities = [
      'brand',
      'primary',
      'blue',
    ...
    print('🎯 Searching for seed colors...');

    // Her öncelik için uygun renkleri ara
    for (final priority in priorities) {
      print('   🔍 Looking for "$priority" colors...');

      // Light tema için 500-600 arası tonlar
      ...
      // Dark tema için 700-800 arası tonlar
    ...
    if (lightSeed == null && colorKeys.isNotEmpty) {
      print('🔄 Using fallback for light seed...');
    ...
    if (darkSeed == null && colorKeys.isNotEmpty) {
      print('🔄 Using fallback for dark seed...');
      final fallback = colorKeys.firstWhere(
    ...
    print('🎨 Final seed color selection:');
  ...
  /// Token key'ini AppColors field adına çevirir
  /// FlutterColorGenerator ile tam uyumlu çalışacak şekilde düzeltildi
  String _tokenKeyToColorName(String tokenKey) {
    print('   🔧 Converting token key: $tokenKey');

    final parts = tokenKey.split('.');
    print('   📋 Parts: $parts');

    // FlutterColorGenerator'ın aynı mantığını kullan
    // Individual colors için (Value Colors)
    if (parts.length >= 3 && (parts[1] == 'color' || parts[1] == 'colors')) {
      // Individual color - _generateTokenName mantığı
      final result = parts
          .skip(1) // İlk parçayı atla
          .map((part) => part.replaceAll(RegExp(r'[^a-zA-Z0-9]'), ''))
          .map((part) => part.toLowerCase())
          .join('_')
          .replaceAll('__', '_');

      print('   ✅ Individual color result: $result');
      return result;
    } else {
      // Themed color - _generateThemeTokenName mantığı
      ...
      print('   ✅ Themed color result: $result');
      return result;
    }
  }
  ...
  Map<String, String?> _findButtonColors(Map<String, dynamic> resolvedTokens) {
    ...
    if (elevatedButtonColor != null || primaryButtonColor != null) {
      print('🔲 Auto-selected button colors:');
